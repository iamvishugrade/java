...
{ 
 public void doPost(HttpServletRequest request,HttpServletResponse response)throws IOException,ServletException
 {
  ...
  String path=getServletContext().getRealPath("/Uploaded files");   
  String ext=".txt";
  try
  {
   MultipartRequest mbr=new MultipartRequest(request,path,500*1024*1024,ext);
   out.println("File uploaded successfully");
  }catch(FileCannotBeUploadException eup){out.println("File can not be upload which have "+ext+" Extension");}
